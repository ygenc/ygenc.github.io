...
var fmi2_useraction_fulltext = "";
var fmi2_sublist = [];

fmi2_sublist.customer = [];
fmi2_sublist.customer.push({
...
fmi2_sublist.banker.push({
  ...
  text: "Find the national interest rates and rate caps",
});
fmi2_sublist.banker.push({
...
fmi2_sublist.consumer = [];
fmi2_sublist.consumer.push({
  url: "/deposit/deposits/",
  text: "Understand how accounts are insured",
});
fmi2_sublist.consumer.push({
...
fmi2_sublist.business = [];
fmi2_sublist.business.push({
  url: "https://ask.fdic.gov/fdicinformationandsupportcenter/s/",
...
fmi2_sublist.analyst = [];
fmi2_sublist.analyst.push({
  url: "/deposit/insurance/",
  text: "Learn about the Deposit Insurance Fund",
});
fmi2_sublist.analyst.push({
...
fmi2_sublist.regulator = [];
fmi2_sublist.regulator.push({
  url: "/regulations/laws/rules/2000-5400.html",
  text: "See deposit insurance regulations",
});
fmi2_sublist.regulator.push({
...
fmi2_sublist.legislative = [];
fmi2_sublist.legislative.push({
  url: "/news/speeches/",
  text: "Read speeches and testimony",
});
fmi2_sublist.legislative.push({
...
fmi2_sublist.jobseeker = [];
fmi2_sublist.jobseeker.push({
  url: "/about/careers/",
  text: "Find a job",
});
fmi2_sublist.jobseeker.push({
...
fmi2_sublist.courtclerk = [];
fmi2_sublist.courtclerk.push({
...
fmi2_sublist.prosprobofficer = [];
fmi2_sublist.prosprobofficer.push({
  url: "/bank/individual/failed/courtclerks.html",
  text: "Find resources for Court Clerks and Law Enforcement Agencies",
});
fmi2_sublist.prosprobofficer.push({
...
fmi2_sublist.investigator = [];
fmi2_sublist.investigator.push({
  url: "/resources/resolutions/bank-failures/failed-bank-list/",
  text: "Find detailed information about a bank failure",
});
fmi2_sublist.investigator.push({
...
function determineAction(user_type, action_url) {
  ...
  for (var i = 0; i < fmi2_sublist[user_type].length; i++) {
    if (fmi2_sublist[user_type][i].url === action_url) {
      user_action_text = fmi2_sublist[user_type][i].text;
      break;
    }
  }
...
*/
function fmi2_update_useraction(form) {
  var user_type = form.value;
  var user_action_dropdown = document.getElementById("fmi2_useraction");
  ...
  for (var i = 0; i < fmi2_sublist[user_type].length; i++) {
    ...
    option.value = fmi2_sublist[user_type][i].url;
    option.innerHTML = fmi2_sublist[user_type][i].text;
    user_action_dropdown.appendChild(option);
  }

  document.getElementById("fmi2_useraction").selectedIndex = 0;
}

function fmi2_useraction_chosen(form) {
  var user_action = form.options[form.selectedIndex].text;
  if (user_action.length > 27) {
    fmi2_useraction_fulltext = user_action;
    form.options[form.selectedIndex].text = user_action.substr(0, 24) + "...";
  }
  //form.blur();
}

function fmi2_useraction_open(i) {
  if (fmi2_useraction_fulltext !== "") {
    var user_action = document.getElementById("fmi2_useraction");
    user_action.options[
      user_action.selectedIndex
    ].text = fmi2_useraction_fulltext;
    fmi2_useraction_fulltext = "";
  }
}

if (document.getElementById("fmi2_usertype").value !== "") {
  fmi2_update_sublist();
}

function fmi2_click() {
  var user = document.getElementById("fmi2_usertype").value;
  var user_action_url = document.getElementById("fmi2_useraction").value;
