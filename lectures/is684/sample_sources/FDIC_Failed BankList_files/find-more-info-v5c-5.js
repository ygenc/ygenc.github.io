...
var fmi4_useraction_fulltext = "";
var fmi4_sublist = [];

fmi4_sublist.customer = [];
fmi4_sublist.customer.push({
...
fmi4_sublist.banker.push({
  ...
  text: "Find national interest rates and rate caps",
});
fmi4_sublist.banker.push({
...
fmi4_sublist.consumer = [];
fmi4_sublist.consumer.push({
  url: "/deposit/deposits/",
  text: "Understand how accounts are insured",
});
fmi4_sublist.consumer.push({
...
fmi4_sublist.business = [];
fmi4_sublist.business.push({
  url: "https://ask.fdic.gov/fdicinformationandsupportcenter/s/",
...
fmi4_sublist.analyst = [];
fmi4_sublist.analyst.push({
  url: "/deposit/insurance/",
  text: "Learn about the Deposit Insurance Fund",
});
fmi4_sublist.analyst.push({
...
fmi4_sublist.regulator = [];
fmi4_sublist.regulator.push({
  url: "/regulations/laws/rules/2000-5400.html",
  text: "See deposit insurance regulations",
});
fmi4_sublist.regulator.push({
...
fmi4_sublist.legislative = [];
fmi4_sublist.legislative.push({
  url: "/news/speeches/",
  text: "Read speeches and testimony",
});
fmi4_sublist.legislative.push({
...
fmi4_sublist.jobseeker = [];
fmi4_sublist.jobseeker.push({
  url: "/about/careers/",
  text: "Find a job",
});
fmi4_sublist.jobseeker.push({
...
fmi4_sublist.courtclerk = [];
fmi4_sublist.courtclerk.push({
...
fmi4_sublist.prosprobofficer = [];
fmi4_sublist.prosprobofficer.push({
  url: "/bank/individual/failed/courtclerks.html",
  text: "Find resources for Court Clerks and Law Enforcement Agencies",
});
fmi4_sublist.prosprobofficer.push({
...
fmi4_sublist.investigator = [];
fmi4_sublist.investigator.push({
  url: "/resources/resolutions/bank-failures/failed-bank-list/",
  text: "Find detailed information about a bank failure",
});
fmi4_sublist.investigator.push({
...
function determineAction(user_type, action_url) {
  ...
  for (var i = 0; i < fmi4_sublist[user_type].length; i++) {
    if (fmi4_sublist[user_type][i].url === action_url) {
      user_action_text = fmi4_sublist[user_type][i].text;
      break;
    }
  }
...
*/
function fmi4_update_useraction(form) {
  var user_type = form.value;
  var user_action_dropdown = document.getElementById("fmi4_useraction");
  ...
  for (var i = 0; i < fmi4_sublist[user_type].length; i++) {
    ...
    option.value = fmi4_sublist[user_type][i].url;
    option.innerHTML = fmi4_sublist[user_type][i].text;
    user_action_dropdown.appendChild(option);
  }

  document.getElementById("fmi4_useraction").selectedIndex = 0;
}

function fmi4_useraction_chosen(form) {
  var user_action = form.options[form.selectedIndex].text;
  if (user_action.length > 27) {
    fmi4_useraction_fulltext = user_action;
    form.options[form.selectedIndex].text = user_action.substr(0, 24) + "...";
  }
  //form.blur();
}

function fmi4_useraction_open(i) {
  if (fmi4_useraction_fulltext !== "") {
    var user_action = document.getElementById("fmi4_useraction");
    user_action.options[
      user_action.selectedIndex
    ].text = fmi4_useraction_fulltext;
    fmi4_useraction_fulltext = "";
  }
}

if (document.getElementById("fmi4_usertype").value !== "") {
  fmi4_update_sublist();
}

function fmi4_click() {
  var user = document.getElementById("fmi4_usertype").value;
  var user_action_url = document.getElementById("fmi4_useraction").value;
